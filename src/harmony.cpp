...
void harmony::dym_lambda_moe_correct_ridge_cpp(){
  ...
  for(int k = 0; k < K; k++){
    ...
    float lambda_dym = find_quan_lambda_cpp(O_k.t(), lambda_win, lambda_quan);
...
void harmony::mid_lambda_moe_correct_ridge_cpp(){
  Z_corr = Z_orig;
  for(int k = 0; k < K; k++){
    arma::rowvec O_k = R.row(k) * Phi.t();
...
void harmony::min_above_moe_correct_ridge_cpp(){
  ...
  for(int k = 0; k < K; k++){
    arma::rowvec O_k = R.row(k) * Phi.t();
    float lambda_dym = find_min_above_lambda_cpp(O_k.t(), lambda_win, lambda_quan);
    all_lambda_dym_vec(k) = lambda_dym;
    arma::vec lambda_dym_vec(lambda.n_rows, arma::fill::value(lambda_dym));
    lambda_dym_vec(0) = 0;
    lambda_dym_mat = arma::diagmat(lambda_dym_vec);
